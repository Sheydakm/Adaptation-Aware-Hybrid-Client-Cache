...
class MyHTTPRequestHandler(BaseHTTPServer.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        entry_id = username = session_id = request_id = time_c = "NULL"
        ...
        try:
            ...
            config_cdash.LOG.info('time for client top :{}'.format(time_c))
        except KeyError:
        ...
        if request in MPD_DICT:
            ...
            request_size = (stat(local_mpd_path).st_size)*8
            ...
            if check_content_server(request):
                ...
                with open(local_file_path, 'rb') as request_file:
                    ...
                    self.wfile.write(request_file.read())
                    request_time = time.time() - start_time
                request_size = (stat(local_file_path).st_size)*8
        ...
        config_cdash.LOG.info('time for client:{}'.format(time_c))
        throughput = float(request_size)/(request_time+float(time_c))
