...
def basic_dash(segment_number, bitrates, average_dwn_time,
    ...
    #download_rate = (recent_download_sizes[len(recent_download_sizes)-1]*8)/ (previous_segment_times[len(previous_segment_times)-1])
    ...
    if download_rate > current_bitrate * config_client.BASIC_UPPER_THRESHOLD:
        ...
        if current_bitrate == bitrates[-1]:
             ...
             for index, bitrate in enumerate(bitrates[1:], 1):
                 if bitrate >= current_bitrate and download_rate < bitrate * config_client.BASIC_UPPER_THRESHOLD:
